import json
import uuid
from validation_services.validate_registration import validate_username, validate_login, validate_password
...
def register_account(validated_username: str, validated_login: str, validated_password: str) -> bool:
    validated_username = validate_username(username=validated_username, accounts_file_dct=acc_file)
    validated_login = validate_login(
        username=validated_username, user_login=validated_login, accounts_file_dct=acc_file)
    validated_password = validate_password(user_password=validated_password, accounts_file_dct=acc_file)
